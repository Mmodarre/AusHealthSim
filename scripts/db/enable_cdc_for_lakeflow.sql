...
-- Step 2: Enable CDC on all user tables in all schemas
DECLARE @TableName NVARCHAR(128);
DECLARE @SchemaName NVARCHAR(128);
DECLARE @SQL NVARCHAR(1000);
DECLARE @CDC_Status INT;
...
BEGIN
    ...
    BEGIN
        SET @SQL = N'EXEC sys.sp_cdc_enable_table ' +
                   N'@source_schema = N''' + @SchemaName + ''', ' +
                   N'@source_name = N''' + @TableName + ''', ' +
                   N'@role_name = NULL, ' +
    ...
    ELSE
    BEGIN
        PRINT 'CDC already enabled on ' + @SchemaName + '.' + @TableName;
    END
